...
ss1[ss1.latency == -41].groupby(['hour','minute','second'])['latency'].agg(['mean','sum','count'])

ss1[ss1.latency == -4041].groupby(['hour','minute','second'])['latency'].agg(['mean','sum','count'])

ss1[ss1.latency == 265].groupby(['hour','minute','second'])['latency'].agg(['mean','sum','count'])
```
</div>

* Delete where time difference goes beyond one second between Kiwoom api time and self-recorded time. In other words, we allow latency +/- ten second at maximum.

<div class = "pycode">
```{python}
ss2 = ss1[abs(ss1.latency) <=10]
