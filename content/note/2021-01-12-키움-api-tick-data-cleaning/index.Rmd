...
## 1. Filter for "Samsung Electronics" Stocks
...
1. Delete rows where "second" does not lie between 0 and 59.

2. Transform data type to str to use substring to make the format like "%Y-%m-%d %H:%M:%S.%f". 
...
ss1 = ss[['time', 'minute', 'price','size','ask1','bid1']]
ss1.head(10)
```
</div>
...
2. Calculate the latency by substracting the tiem provided by Kiwoom dataset and self-recorded time at the moment of processing the data delivered through the Kiwoom API.

<div class = "pycode">
```{python}
# Task 2 & 3
latency = ss['timestamp'].apply(lambda x: int(str(int(x*1000000))[8:14])) - ss['trade_date']
print(latency.value_counts())
ss1['latency'] = latency
ss1.head()
```
</div>
...
3. Check the time where large discrepancy happened!
...
4. Delete where time difference goes beyond one second between Kiwoom api time and self-recorded time. In other words, we allow latency +/- one second at maximum.
